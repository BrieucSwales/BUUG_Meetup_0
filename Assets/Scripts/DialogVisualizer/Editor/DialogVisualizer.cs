...
public partial class DialogVisualizer : EditorWindow
{
    DialogRenderer m_Renderer;
    ReingoldTilford m_Layout;

    Dictionary<DialogPlayable, string> m_Graphs = new Dictionary<DialogPlayable, string>();
    DialogPlayable m_CurrentGraph = null;

    const float k_ToolbarHeight = 17f;

    [MenuItem("Window/Dialog Visualizer")]
    public static void ShowWindow()
    {
        EditorWindow.GetWindow<DialogVisualizer>();
    }

    private DialogVisualizer()
    {
        DialogVisualizerClient.instance.updateGraph = OnUpdateGraph;
