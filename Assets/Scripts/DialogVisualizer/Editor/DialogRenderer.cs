...
public class DialogRenderer : ITreeRenderer
{
    static Color k_EdgeColorMin = new Color(1.0f, 1.0f, 1.0f, 0.1f);
    static Color k_EdgeColorMax = Color.white;
    static Color k_LegendBackground = new Color(0, 0, 0, 0.1f);

    const float k_BorderSize = 32f;
    const float k_LegendFixedOverheadWidth = 30;
    const float k_DefaultMaximumNormalizedNodeSize = 0.8f;
    const float k_DefaultMaximumNodeSizeInPixels = 100.0f;
    const float k_DefaultAspectRatio = 1;

    bool m_UseCustomDrawingMethods = true;
    GUIStyle m_NodeLabelStyle;
    GUIStyle m_LegendLabelStyle;
    GUIStyle m_SubTitleStyle;

    Dictionary<Type, NodeTypeLegend> m_LegendForType = new Dictionary<Type, NodeTypeLegend>();
    Dictionary<Type, MethodInfo> m_CustomDrawingMethodForType = new Dictionary<Type, MethodInfo>();

    public static Texture2D dialog_bubble;

    public static DialogPlayable selected_node;
    
    static string[] k_PredefinedNodeStyles = new string[]
    {
        "flow node 1", // blue
        "flow node 4", // yellow
        "flow node 6", // red
    ...
    const string k_DefaultNodeStyle = "flow node 0";

    Texture2D m_ColorBar = null;
    ...
    public void Draw(ILayoutAlgorithm layout, Rect totalDrawingArea, NodeConstraints nodeConstraints)
    {
        ...
        var sidebar_area = new Rect(totalDrawingArea);
        sidebar_area.width = EstimateLegendWidth() + k_BorderSize * 2;
        sidebar_area.x = drawingArea.xMax - sidebar_area.width;
        drawingArea.width -=  sidebar_area.width;

        DrawGraph(layout, drawingArea, nodeConstraints);
        DrawSidebar(sidebar_area);
    }
    ...
    void InitializeStyles()
    {
        ...
        DialogRenderer.dialog_bubble = (Texture2D)Resources.Load( "dialog_bubble", typeof( Texture2D ) );
    }

    void PrepareLegend(Vertex[] vertices)
    {
    ...
    void FindCustomDrawingFunctions()
    {
    ...
    void DrawSidebar(Rect sidebar_area)
    {
        EditorGUI.DrawRect(sidebar_area, k_LegendBackground);

        // Add a border around sidebar area
        sidebar_area.x += k_BorderSize;
        sidebar_area.width -= k_BorderSize * 2;
        sidebar_area.y += k_BorderSize;
        sidebar_area.height -= k_BorderSize * 2;

        GUILayout.BeginArea(sidebar_area);
        GUILayout.BeginVertical();

        // Selected dialog info
        GUILayout.Label("Info", m_SubTitleStyle);
        if ( DialogRenderer.selected_node == null ) {
            GUILayout.Label( "No selection", m_LegendLabelStyle );
        } else {
            GUILayout.Label( selected_node.name, m_LegendLabelStyle );
            EditorGUILayout.TextArea( selected_node.content, m_LegendLabelStyle );
        }
        ...
        DrawEdgeWeightColorBar(sidebar_area.width);
    ...
    void DrawGraph(ILayoutAlgorithm layout, Rect drawingArea, NodeConstraints nodeConstraints)
    {
    ...
    Vector2 ComputeNodeSize(Vector2 scale, NodeConstraints nodeConstraints)
    {
    ...
    Vector2 ScaleVertex(Vector2 v, Vector2 offset, Vector2 scaleFactor)
    {
    ...
    void GetTangents(Vector2 start, Vector2 end, out Vector3[] points, out Vector3[] tangents)
    {
    ...
    void DrawEdge(Vector2 a, Vector2 b, float weight)
    {
